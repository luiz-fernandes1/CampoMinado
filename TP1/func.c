...
void salvaJogo(char jogoResolvido[100][100], char jogoLimpo[100][100], int linha, int coluna){
    FILE *arquivo = fopen("out.jogo", "w");
    fprintf(arquivo, "%d %d\n\n", linha, coluna);
    for (int i = 0; i < linha; i++) {
        for (int j = 0; j < coluna; j++) {
            fprintf(arquivo, "%c ", jogoResolvido[i][j]);
        }
        fprintf(arquivo, "\n");
    }
    fclose(arquivo);

    // FILE *outroarquivo = fopen("out.txt", "w");
    // fprintf(outroarquivo, "%d %d\n\n", linha, coluna);
    //
    // for (int i = 0; i < linha; i++) {
    //     for (int j = 0; j < coluna; j++) {
    //         if (jogoLimpo[i][j] != 'x' && jogoLimpo[i][j] != '-') {
    //             jogoLimpo[i][j] = 'o';
    //         }
    //     }
    // }
    ...
    // for (int i = 0; i < linha; i++) {
    //     for (int j = 0; j < coluna; j++) {
    //         fprintf(arquivo, "%c ", jogoLimpo[i][j]);
    //     }
    //     fprintf(arquivo, "\n");
    // }
    // fclose(outroarquivo);
